...
import { enrichCenter } from '@/features/disposal/utils/disposalFilters';
...
export const DisposalCenterDetailsPage: React.FC = () => {
  ...
  const center: DisposalCenterView = useMemo(() => {
    let raw: DisposalCenter = MOCK_DISPOSAL_CENTERS[0];
    if (id) {
      ...
      if (byId) {
        ...
        if (!isNaN(num) && num >= 1 && num <= MOCK_DISPOSAL_CENTERS.length) {
          raw = MOCK_DISPOSAL_CENTERS[num - 1];
        }
      }
    }
    return enrichCenter(raw);
  }, [id]);
  ...
  return (
    <ResidentLayout activeItem="disposal-centers" contentClassName="max-w-none p-0 sm:p-0 lg:p-0">
      ...
      <Container size="full" className="max-w-6xl mx-auto px-6 py-6 flex-1 space-y-6">
        ...
        {/* On-Site Disposal Instructions (Official Protocol) */}
        ...
        {/* Location Preview with Interactive Map */}
        <div className="bg-surface rounded-2xl border border-border p-6 space-y-4 shadow-card">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
